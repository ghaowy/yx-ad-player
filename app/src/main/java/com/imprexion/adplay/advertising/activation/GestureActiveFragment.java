...
//        nextAfterStandRight();
    ...
    private void waveActiveSucess() {
        ...
        mHandler.sendMessageDelayed(mMessage, 1000);//此处提示摇手3000ms(自定义摇多久，是循环的帧动画)

        mMessage = mHandler.obtainMessage();
        mMessage.what = ELEPHANT_EXIT;
        mHandler.sendMessageDelayed(mMessage, 1000 + 1000);//此处提示摇手3000ms(自定义摇多久，是循环的帧动画)+注水的一半时间1000ms =4000ms，后开始退出动画
    }
